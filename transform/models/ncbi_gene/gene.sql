MODEL (
  ...
  column_descriptions (
    ...
    taxon_id = 'NCBI Taxonomy id (bioregistry prefix `ncbitaxon`). Bare local id.',
    gene_type = 'NCBI''s `type_of_gene` (protein-coding, pseudo, ncRNA, biological-region, …).'
  ),
  audits (ncbi_gene_gene_gene_id_is_unique, ncbi_gene_gene_no_newentry_placeholder_rows_survive)
);
